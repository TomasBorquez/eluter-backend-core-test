...
export async function createAccount(_db = db) {
  const [result] = await _db<
    Pick<Balance, 'id'>[]
  >`INSERT INTO balances (balance) VALUES (0) RETURNING id`;
  return result.id;
}
...
export async function getBalance(accountId: string, _db = db) {
  const [result] = await _db<
    Balance[]
  >`SELECT balance FROM balances WHERE id = ${accountId}`;
  return result;
}
...
export async function debit(
  ...
  const { balance } = await getBalance(accountId, _db);
  const newBalance = (parseFloat(balance) - parseFloat(amount)).toString();
  const res =
    await _db`UPDATE balances SET balance = ${newBalance} WHERE id = ${accountId}`;
...
export async function atomicDebit(
  ...
  const res =
    await _db`UPDATE balances SET balance = balance - ${amount} WHERE id = ${accountId}`;
  await hooks?.postDebit?.waitFor();
  return res;
}
